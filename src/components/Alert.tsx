...
const Alert = ({ type = 'info', message = '' }: AlertProps) => {
  ...
  const portalElem = useRef<any>(null);
  const [mounted, setMounted] = useState(false);

  useEffect(() => {
    portalElem.current = document.querySelector<HTMLElement>('#portal');
    setMounted(true);
  }, []);

  return mounted && portalElem.current
    ? createPortal(
        <div
          ref={alertElem}
          className={`fixed left-1/2 top-8 mb-4 flex -translate-x-1/2 rounded-lg px-2 py-3 drop-shadow-xl md:px-4 md:py-6 ${
            type === 'danger'
              ? 'bg-red-50'
              : type === 'success'
              ? 'bg-green-50'
